...
function SetupBanner({ onDismiss }) {
  ...
  estado text, status text, area_total text, ano_safra text, telefone text, email text, gerente text, consultor text,
  lat text, lng text,
...
export default function AgroMap() {
  ...
  useEffect(() => {
    ...
    satelite.addTo(map);
    L.control.layers({ "Satelite": satelite, "Mapa": ruas }, {}, { position: "topright", collapsed: false }).addTo(map);
    mapRef.current = map;
